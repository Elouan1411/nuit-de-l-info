...
function spawnTrash() {
    ...
    trash.addEventListener("click", () => {
        ...
        trash.addEventListener(
            ...
            { once: true }
        ); // L'événement ne doit se déclencher qu'une fois
    });
...
function endGame() {
    ...
    if (score > 45) {
        message.textContent =
            "Félicitations, vous avez gagné ! Aux abords de la terre, l'océan va et vient au rythme des marées, lissant le sable des plages, et emportant ce qu'il y trouve. Mais il emporte aussi avec lui les déchets que nous y laissons... Heureusement, nous pouvons agir pour éviter cela";
    } else {
...
toggleButton.addEventListener("click", () => {
    if (!isMessage) {
        ...
        toggleButton.textContent =
            "Attention  la marée monte et la plage est pleine de déchets ! Ramasser le plus de déchets possible pour ralentir la marée et gagner des points. Si les déchets se font emporter par l'océan vous perdez des points... Alors attention et ramassez vite !!";
